...
for name, ticker in equity_symbols.items():
    ...
    if score >= 70:
        ...
        send_telegram(
            ...
            f"T1: ₹{round(entry*1.1,2)}\nT2: ₹{round(entry*1.2,2)}\n"
            f"Confidence: {score}%\n\nNews Check: Confirm before entry"
        )
...
for name in list(state.keys()):
    ...
    if not trade["t1_hit"] and price >= trade["t1"]:
        trade["t1_hit"] = True
        trade["stop_loss"] = trade["entry"] * 1.05

        send_telegram(
            f"🟢 UPDATE — {name} (SWING)\n\n"
            "Target 1 Achieved ✅\n\n"
            "Action:\n• Book 50% quantity\n• Hold remaining\n\n"
            f"SL moved to ₹{round(trade['stop_loss'],2)}"
        )
    ...
    if trade["t1_hit"]:
        ...
        if momentum and not trade["dynamic"]:
            trade["dynamic"] = True
            trade["t1"] = trade["t2"]
            ...
            send_telegram(
                f"🔵 MOMENTUM UPDATE — {name}\n\n"
                f"Targets revised:\n"
                f"• New T1: ₹{round(trade['t1'],2)}\n"
                f"• New T2: ₹{round(trade['t2'],2)}\n"
